...
class SimpleMLP(nn.Module):
    def __init__(self, input_shape, num_classes=12):
        ...
        self.layers = self._make_layers(input_shape)
        
        # If you want a classification layer at the end
        # self.classifier = nn.Linear(self.cfg[-1], num_classes)

    def _make_layers(self, input_shape):
        layers = []
        in_features = input_shape
        
        for out_features in self.cfg:
            layers.append(BlockMLP(in_features, out_features))
...
# Function to create the SimpleMLP model
def SimpleMLPmodel(input_shape=80, num_classes=12):
    return SimpleMLP(input_shape, num_classes=num_classes)
...
def create_backbone(config_dict, name, num_classes=10, block='BasicBlock'):
    if(name == 'VGG'):
        ...
        net = SimpleMLPmodel(input_shape=config_dict["input_features"], num_classes=num_classes)
...
class simclr(nn.Module):
    def __init__(self, config_dict, bbone_arch):
        ...
        self.backbone = create_backbone(config_dict, bbone_arch, num_classes=0)
        self.projector = projection_MLP_simclr(self.backbone.output_dim, hidden_dim=512, out_dim=512)

    def forward(self, x1, x2, x3=None, deg_labels=None, is_global=False):
        N = x1.shape[0]
...
class MLP_BYOL(nn.Module):
    def __init__(self, in_dim, hidden_dim=1024, out_dim=512, is_pred=False):
        ...
        self.layer2 = nn.Linear(hidden_dim, out_dim, bias=is_pred)
...
class byol(nn.Module):
    def __init__(self, config_dict, bbone_arch):
        ...
        # Target model
        self.target_backbone = create_backbone(config_dict, bbone_arch, num_classes=0)
...
class orchestra(nn.Module):
    ...
    def reset_memory(self, data, device='cuda:0'):
        ...
        # print("-------xxx------", proj_bank.size(), self.m_size)
    ...
    def forward(self, x1, x2, x3=None, deg_labels=None):
        ...
        # print("deg_labels",  deg_labels)
        if deg_labels.size(1) != 0: 
            print("AAAAAAAAAA", deg_labels)
...
# Projector 
class projection_MLP_custom(nn.Module):
    def __init__(self, in_dim, hidden_dim=512, out_dim=512):
        super(projection_MLP_custom, self).__init__()
        self.layer1 = nn.Linear(in_dim, hidden_dim, bias=False)
        self.layer1_bn = nn.BatchNorm1d(hidden_dim, affine=True)
        self.layer2 = nn.Linear(hidden_dim, out_dim, bias=False)
        self.layer2_bn = nn.BatchNorm1d(out_dim, affine=False)
...
# Predictor
class prediction_MLP_custom(nn.Module):
    def __init__(self, in_dim, hidden_dim=512, out_dim=512):
        super(prediction_MLP_custom, self).__init__()
        self.layer1 = nn.Linear(in_dim, hidden_dim, bias=False)
        self.layer1_bn = nn.BatchNorm1d(hidden_dim, affine=True)
        self.layer2 = nn.Linear(hidden_dim, out_dim, bias=False)
        self.layer2_bn = nn.BatchNorm1d(out_dim, affine=False)
...
class mymodel(nn.Module):
    def __init__(self, config_dict, bbone_arch):
        ...
        ### Online Model
        self.backbone = create_backbone(config_dict, bbone_arch, num_classes=0)
        self.projector = projection_MLP_custom(in_dim=self.backbone.output_dim, out_dim=512)
        self.predictor = prediction_MLP_custom(in_dim=512, out_dim=512)
        ...
        self.target_projector = projection_MLP_custom(in_dim=self.backbone.output_dim, out_dim=512)
        self.target_predictor = prediction_MLP_custom(in_dim=512, out_dim=512)
    ...
    # Main training function
    def forward(self, x1, x2, x3=None, deg_labels=None, tZ1_local=None, tZ2_local=None, random_x=None, is_global=False):
        if is_global:
            N = x1.shape[0]
            C = self.local_centroids.weight.data.detach().clone().T
            ...
            with torch.no_grad():
                # JSD loss
                L_js = 0
                L_js = self.js_loss(self.target_predictor(tZ1_local), self.target_predictor(tZ2_local),  self.target_projector(self.target_backbone(random_x)))
                # print("local", L_js)
                
                tZ1 = F.normalize(self.target_predictor(tZ1_local), dim=1)
            ...
            with torch.no_grad():
                ...
                # JSD loss
                L_js = 0
                L_js = self.js_loss(self.target_projector(self.target_backbone(x1)), self.target_projector(self.target_backbone(x2)),  self.target_projector(self.target_backbone(random_x)))
                # print("local", L_js)
            ...
            return L, self.target_projector(self.target_backbone(x1)), self.target_projector(self.target_backbone(x2))
